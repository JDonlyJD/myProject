 ...
 function selectData(pageNum){   //pageNum을 넘겨주면 currentPage로 지정
      ...
      $.ajax({
         ...
         success:function(param){
            ...
            output += '		</div>'; //"sub-item" end
            output += '</div>';//"item" end
           // output += '<hr id="hr_cate"  noshade width="100%">';               
            output += '<br>';
         ...
         */
         ...
         error:function(){
            ...
            alert('네트워크 오류발생 -101 !'); //=> 발생시에는 이클립스의 콘솔 에러메시지를 보기
         }         
      });
   }
...
/*---- 화면에 댓글처럼 등록되는 카테고리 */
   ...
   $('#re_form').submit(function(event){         
      ...
      */
      ...
      let send={
         ...
         cate_status : $('#cate_status:checked').val()
         }
   //console.log
   console.log('화면댓글');
   console.log(send);
   ...
   function initForm(){
      $('textarea').val('');
      $('#re_first .letter-count').text('10/10');
   }   
   ...
   $(document).on('keyup','textarea',function(){                              
      ...
      if(inputLength > 10){   //* 300자를 넘어선 경우 => susbstring으로 문자열 잘라냄
         $(this).val($(this).val().substring(0,10));
         
      }else{   //* 10자 이하인 경우 => 몇자남았는지 알려줌 (ex. 2/10)
         let remain = 10 - inputLength;
         remain += '/10';
